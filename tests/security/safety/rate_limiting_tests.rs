...
mod tests {
    ...
    use tw_api::rate_limit::{
        ApiRateLimiter, ApiRateLimitError,
        LoginRateLimiter, RateLimitError,
        WebhookRateLimiter, WebhookRateLimitError,
    };
...
fn test_webhook_global_rate_limit() {
    ...
    let sources = ["source1", "source2", "source3", "source4", "source5", "source6"];
...
fn test_webhook_queue_overflow_protection() {
    ...
    assert!(
        matches!(
            result,
            Err(WebhookRateLimitError::QueueOverflow { current_depth: 3, max_depth: 3 })
        ),
...
fn test_webhook_stats_accuracy() {
    ...
    assert_eq!(stats.rate_limited_requests, 1, "One request should be rate limited");
...
fn test_api_rate_limiter_lru_eviction() {
    ...
    assert_eq!(limiter.ip_eviction_count(), 2, "Two IP evictions should occur");
    ...
    assert_eq!(limiter.user_eviction_count(), 2, "Two user evictions should occur");
}
...
fn test_webhook_rate_limiter_lru_eviction() {
    ...
    assert_eq!(limiter.tracked_sources(), 3, "Source cache should be bounded");
    assert_eq!(limiter.source_eviction_count(), 2, "Two source evictions should occur");
}
...
fn test_webhook_empty_source_name() {
    ...
    assert!(matches!(result, Err(WebhookRateLimitError::PerSourceLimitExceeded { source }) if source.is_empty()));
}
...
fn test_webhook_unicode_source_names() {
    ...
    for source in sources {
        for _ in 0..3 {
            assert!(limiter.check(source).is_ok(), "Unicode source {} should work", source);
        }
    }
}
...
fn test_clear_ip_resets_quota() {
    ...
    assert!(limiter.check(ip).is_ok(), "IP should have fresh quota after clear");
}
