...
use tw_actions::registry::{ActionError, ActionRegistry, SignedRollbackData};
...
fn test_signed_rollback_data_creation() {
    ...
    let signed =
        SignedRollbackData::new(data.clone(), "isolate_host", execution_id, encryption_key)
...
fn test_verification_detects_tampered_execution_id() {
    ...
    let mut signed =
        SignedRollbackData::new(data, "test_action", original_execution_id, encryption_key)
            .expect("Should create signed data");
...
fn test_different_keys_produce_different_signatures() {
    ...
    let signed2 =
        SignedRollbackData::new(data, "action", execution_id, key2).expect("Should sign with key2");
...
fn test_same_key_same_data_produces_same_signature() {
    ...
    let signed1 =
        SignedRollbackData::new(data.clone(), "action", execution_id, key).expect("Should sign");
