...
async fn generate_report(
    ...
    match query.format.to_lowercase().as_str() {
        ...
        "pdf" => {
            let pdf_content = generate_pdf_report(&report_data)?;
            ...
            Ok((
                ...
                [
                    ...
                    (
                        header::CONTENT_DISPOSITION,
                        &format!("attachment; filename=\"incident_{}_report.pdf\"", id),
                    ),
                ],
                pdf_content,
            )
...
/// Generate a simple, dependency-free PDF report.
///
/// This intentionally creates a compact single-page PDF with key report details.
/// It avoids external binaries/libraries while providing native PDF export.
fn generate_pdf_report(report: &ReportData) -> Result<Vec<u8>, ApiError> {
    const MAX_CHARS_PER_LINE: usize = 90;
    const MAX_LINES: usize = 52;

    let mut lines = build_pdf_lines(report, MAX_CHARS_PER_LINE);
    if lines.len() > MAX_LINES {
        lines.truncate(MAX_LINES - 1);
        lines.push("... output truncated in PDF export ...".to_string());
    }

    let mut text_stream = String::from("BT\n/F1 11 Tf\n14 TL\n50 780 Td\n");
    for (idx, line) in lines.iter().enumerate() {
        text_stream.push('(');
        text_stream.push_str(&pdf_escape_text(line));
        text_stream.push_str(") Tj\n");
        if idx + 1 < lines.len() {
    ...
    let content_length = text_stream.len();
...
fn build_pdf_lines(report: &ReportData, max_chars_per_line: usize) -> Vec<String> {
    let mut raw_lines = vec![
        format!("Investigation Report: {}", report.metadata.incident_id),
        format!("Generated: {}", report.metadata.generated_at),
        format!("Generated by: {}", report.metadata.generated_by),
        "".to_string(),
        ...
        format!(
            ...
            (report.verdict.confidence * 100.0).round()
        ),
        format!(
            "Severity: {} | Risk Score: {}",
            report.verdict.severity_display, report.verdict.risk_score
        ),
        format!("Alert Source: {}", report.alert.source),
    ];

    if !report.recommended_actions.is_empty() {
        raw_lines.push("".to_string());
        raw_lines.push("Recommended Actions".to_string());
        for (idx, action) in report.recommended_actions.iter().take(10).enumerate() {
            ...
            raw_lines.push(format!("   {}", action.reason));
        }
    }

    if !report.indicators.is_empty() {
        raw_lines.push("".to_string());
        raw_lines.push("Top Indicators".to_string());
        for indicator in report.indicators.iter().take(10) {
            raw_lines.push(format!(
                "- {}: {} ({})",
                indicator.indicator_type.to_uppercase(),
                indicator.value,
                indicator.verdict
            ));
        }
    ...
    if !report.reasoning.trim().is_empty() {
        raw_lines.push("".to_string());
        raw_lines.push("Reasoning".to_string());
        raw_lines.push(report.reasoning.clone());
    }

    raw_lines
        .into_iter()
        .flat_map(|line| wrap_text(&line, max_chars_per_line))
        .collect()
}
...
fn wrap_text(text: &str, max_chars_per_line: usize) -> Vec<String> {
    ...
    for paragraph in text.split('\n') {
        ...
        for word in paragraph.split_whitespace() {
            ...
            if current.len() + 1 + word.len() <= max_chars_per_line {
                current.push(' ');
                current.push_str(word);
            } else {
...
mod tests {
    ...
    fn sample_report_data() -> ReportData {
        ReportData {
            metadata: ReportMetadataResponse {
                ...
                generated_at: "2026-02-07T12:00:00Z".to_string(),
                generated_by: "analyst@example.com".to_string(),
                report_version: "1.0".to_string(),
                format: "pdf".to_string(),
            },
            executive_summary: "Investigation confirms malicious email activity.".to_string(),
            verdict: VerdictSummary {
                verdict: "true_positive".to_string(),
                verdict_display: "True Positive".to_string(),
                confidence: 0.93,
                calibrated_confidence: Some(0.91),
                severity: "high".to_string(),
                severity_display: "High".to_string(),
                risk_score: 84,
            },
            alert: AlertSummary {
                alert_id: "alert-123".to_string(),
                source: "M365".to_string(),
                alert_type: Some("phishing".to_string()),
                title: Some("Suspicious Email".to_string()),
                created_at: "2026-02-07T11:58:00Z".to_string(),
            },
            timeline: Vec::new(),
            evidence: Vec::new(),
            evidence_summary: EvidenceSummaryData {
                ...
                data_types_found: Vec::new(),
            },
            mitre_techniques: Vec::new(),
            indicators: vec![IndicatorEntry {
                indicator_type: "email".to_string(),
                value: "attacker@example.com".to_string(),
                verdict: "malicious".to_string(),
                context: None,
            }],
            recommended_actions: vec![ActionEntry {
                order: 1,
                action: "Quarantine related messages".to_string(),
                priority: "high".to_string(),
                reason: "Contain spread quickly".to_string(),
                requires_approval: true,
            }],
            reasoning: "The sender domain failed DMARC and matched prior threat intel.".to_string(),
            raw_alert_data: None,
            enrichments: None,
            audit_log: Vec::new(),
        }
    ...
    #[test]
    fn test_pdf_escape_text() {
        assert_eq!(pdf_escape_text("(test)\\value"), "\\(test\\)\\\\value");
    }

    #[test]
    fn test_generate_pdf_report_structure() {
        let report = sample_report_data();
        let pdf = generate_pdf_report(&report).expect("PDF generation should succeed");
